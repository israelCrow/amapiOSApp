//
//  CompanyPitchesTutorialView.swift
//  Amap
//
...
class CompanyPitchesTutorialView: UIView {
  ...
  private func createRightBottomLabel() {
    ...
    let frameForLabel = CGRect.init(x: 0.0,
                                    y: 0.0,
                                    width: 164.0 * UtilityManager.sharedInstance.conversionWidth,
                                    height: CGFloat.max)
    ...
    let stringWithFormat = NSMutableAttributedString(
      string: "Aquí verás los pitches que te han asignado",
  ...
  @objc private func tapToThisView() {
    ...
    let secondTutorialPitches = CompanyPitchesSecondTutorialView.init(frame: CGRect.init())
    let rootViewController = UtilityManager.sharedInstance.currentViewController()
    
    self.removeFromSuperview()
    NSUserDefaults.standardUserDefaults().setBool(true, forKey: UtilityManager.sharedInstance.kNotToShowCompanyPitchesTutorial + UserSession.session.email)
    
    rootViewController.view.addSubview(secondTutorialPitches)
