//
//  DidProjectActivePitchSurveyView.swift
//  Amap
//
...
protocol DidProjectActivePitchSurveyViewDelegate {
  
  func didProjectActivePitchSurveyNextButtonPressedSoShowScreenWithTag(valueSelected: String, nextScreenToShowWithTag: Int)
  
}

class DidProjectActivePitchSurveyView: UIView, CustomSegmentedControlWithTitleViewDelegate {
  ...
  private let ifAnswerIsYesGoTo = -1
  private let ifAnswerIsNoGoTo = 12
  
  var delegate: DidProjectActivePitchSurveyViewDelegate?
  ...
  private func createTitleLabel() {
    ...
    let stringWithFormat = NSMutableAttributedString(
      string: "¿Se activó el proyecto?", //EditPitchesConstants.DidSignContactPitchSurveyView.titleLabelText,
  ...
  private func createDidYouShowYourProposalView() {
    ...
    didProjectActiveView = CustomSegmentedControlWithTitleView.init(frame: frameForView,
                                                                              title: "", //"¿Se activó el proyecto?",
                                                                              image: nil,
                                                                              segmentsText: segmentsArray)
    let originalSegmentControlFrame = didProjectActiveView.mainSegmentedControl.frame
    ...
    didProjectActiveView.mainSegmentedControl.frame = newFrame
    didProjectActiveView.mainSegmentedControl.setWidth(kSpaceInSegments, forSegmentAtIndex: 1)
    didProjectActiveView.mainSegmentedControl.setEnabled(false, forSegmentAtIndex: 1)
    didProjectActiveView.delegate = self
    
    self.addSubview(didProjectActiveView)
  ...
  @objc private func nextButtonPressed() {
    ...
    let selectedAnswer = didProjectActiveView.returnValueSelectedFromSegmentControl()
    
    if selectedAnswer == "Sí" {
      
      self.delegate?.didProjectActivePitchSurveyNextButtonPressedSoShowScreenWithTag(selectedAnswer, nextScreenToShowWithTag: ifAnswerIsYesGoTo)
      
    } else {
      
      self.delegate?.didProjectActivePitchSurveyNextButtonPressedSoShowScreenWithTag(selectedAnswer, nextScreenToShowWithTag: ifAnswerIsNoGoTo)
