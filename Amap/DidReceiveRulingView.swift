...
protocol DidReceiveRulingViewDelegate {
...
class DidReceiveRulingView: UIView, CustomSegmentedControlWithTitleViewDelegate {
  ...
  private let ifAnswerIsYesGoTo = 3
  private let ifAnswerIsNoGoTo = 6
  
  var delegate: DidReceiveRulingViewDelegate?
  ...
  private func createTitleLabel() {
    ...
    let stringWithFormat = NSMutableAttributedString(
      string: "¿Recibiste fallo?",  //EditPitchesConstants.DidYouShowYourProposalView.titleLabelText,
  ...
  @objc private func nextButtonPressed() {
    ...
    if selectedAnswer == "Sí" {
      ...
      self.delegate?.didReceiveRulingNextButtonPressedSoShowScreenWithTag(selectedAnswer, nextScreenToShowWithTag: ifAnswerIsYesGoTo)
      
    } else {
      
      self.delegate?.lookForWinnerOfPitch()
